    ...
    <div className="mt-6 w-full pb-20 ">
      {items.map((item, index) => (
        <div key={index} className={"pb-4"}>
          <h4 className="mb-1 rounded-md p-2 text-sm font-semibold">
            {item.title}
          </h4>
          {item?.items && (
            <DocsSidebarNavItems
      ...
      {items.map((item, index) =>
        item.href && !item.disabled ? (
          ...
          >
            ...
            {item.label && (
              <span className="relative z-10 ml-2 rounded-md bg-green-400 px-1.5 py-0.5 text-xs leading-none text-[#000000] no-underline group-hover:no-underline">
                {item.label}
              </span>
            )}
            {item.paid && (
